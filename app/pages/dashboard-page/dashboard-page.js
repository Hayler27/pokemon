...
class DashboardPage extends intl(cellsPage(LitElement)) {
  static get is() {
  ...
  updated(props) {
    ...
    if (props.has('_accounts') && this._accounts.length) {
      this._selectAccounts
        .querySelectorAll('bbva-web-form-option-filter')
        .forEach((option) => {
          if (option.selected) {
            this._accountId = option.value;
            this._selectAccount = this._accounts.filter(
              (account) => account.account === this._accountId
            )[0];
          }
        });
  ...
  render() {
    return html`
      <demo-web-template page-title="Dashboard" reset-detail-on-state-change>
          ...
          return html`
            ...
            >
              <bbva-web-clip-entities
                asset="${account.entity
                  .substring(account.entity.indexOf(':') + 1)
                  .toLowerCase()}"
                slot="clip"
              >
                ${account.entity
              ...
              <span slot="description"
                >${mask(account.account, '•', true, 4)}</span
              >

              ${account.alias ||
              account.title.substring(0, account.title.lastIndexOf(' '))}
            </bbva-web-form-option-filter>
          `;
        })}
      </bbva-web-form-select-filter>
    ...
    return html`
      <div class="content-detail">
        <bbva-web-expandable-accordion
          header-title="${this.t(this._i18nKeys.accountDetail)}"
          size="s"
        >
          <ul class="account-detail">
            <li>
              ...
              >
                <span slot="label"
                  >${this.t(this._i18nKeys.accountNumber)}</span
                >
                <span slot="description"
                  >${this._getFormatAccountNumber(
            ...
            <li>
              <bbva-web-list-item-simple>
                ...
                <span slot="description"
                  >${this._selectAccount.entityName}</span
                >
              </bbva-web-list-item-simple>
            </li>
          </ul>
